        			...
        			<div class="form-group">
	        			...
	        			<select name="id_bulan_pembayaran" id="id_bulan_pembayaran" class="form-control">
	        				<?php if (isset($_POST['btnLaporanPemasukkan'])): ?>
	        					<option value="<?= $fetch_sql['id_bulan_pembayaran']; ?>"><?= ucwords($fetch_sql['nama_bulan']); ?> | <?= $fetch_sql['tahun']; ?> | Rp. <?= number_format($fetch_sql['pembayaran_perbulan']); ?></option>
		        				<option disabled>----</option>
	        				<?php endif ?>
		        			<?php foreach ($bulan_pembayaran as $dbp): ?>
		        				<option value="<?= $dbp['id_bulan_pembayaran']; ?>"><?= ucwords($dbp['nama_bulan']); ?> | <?= $dbp['tahun']; ?> | Rp. <?= number_format($dbp['pembayaran_perbulan']); ?></option>
		        			<?php endforeach ?>
	        			</select>
        			</div>
        			<div class="form-group">
        ...
        <?php if (isset($_POST['btnLaporanPemasukkan'])): ?>
        	...
        	<div class="row m-1">
	        	<div class="col-lg m-1">
	        		...
	        		<h3 class="text-left mb-3">Laporan Pada Bulan: <?= ucwords($fetch_sql['nama_bulan']); ?>, Tahun: <?= $fetch_sql['tahun']; ?>, Pembayaran Perbulan: Rp. <?= number_format($fetch_sql['pembayaran_perbulan']); ?></h3>
	        				...
	        				<tbody>
	        					...
	        					<?php foreach ($sql as $ds): ?>
	        						<tr>
	        							...
	        							<?php if ($ds['kas'] == $ds['pembayaran_perbulan']): ?>
					                      <td class="text-success"><?= number_format($ds['kas']); ?></td>
					                    <?php else: ?>
					                      <td class="text-danger"><?= number_format($ds['kas']); ?></td>
					                    <?php endif ?>
	        						</tr>
	        					<?php endforeach ?>
	        				</tbody>
	        ...
	        <div class="row mx-1 mb-1 mt-0">
    			<div class="col-lg-4">
    				<?php 
    					$jml_uang_kas = mysqli_fetch_assoc(mysqli_query($conn, "SELECT *, sum(kas) as jml_uang_kas FROM uang_kas INNER JOIN bulan_pembayaran ON bulan_pembayaran.id_bulan_pembayaran = uang_kas.id_bulan_pembayaran WHERE bulan_pembayaran.id_bulan_pembayaran = '$id_bulan_pembayaran'"));
    				?>
