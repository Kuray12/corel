...
function logout() {
	setAlert("Anda sudah logout!", "Berhasil Logout!", "success");
	header("Location: login.php");
}
...
function checkJabatan() {
	...
	if ($id_jabatan !== '1') {
		setAlert("Akses Ditolak!", "Anda tidak dapat menghapus data kecuali administrator!", "error");
     	header("Location: index.php");
	} else {
		return true;
	}
...
function addUser($data) {
	...
	if (mysqli_fetch_assoc($query)) {
		setAlert("Gagal menambahkan pengguna!", "Username sudah digunakan!", "error");
     	return header("Location: user.php");
	} else {
		$password = htmlspecialchars($data['password']);
		$password_verify = htmlspecialchars($data['password_verify']);
		if ($password !== $password_verify) {
			setAlert("Gagal menambahkan pengguna!", "Password tidak sama dengan password verify!", "error");
	     	return header("Location: user.php");
		} else {
...
function addBulanPembayaran($data) {
	...
	$pembayaran_perbulan = htmlspecialchars($data['pembayaran_perbulan']);
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		setAlert("Gagal menambahkan " . ucwords($nama_bulan) . "!", "Karena " . ucwords($nama_bulan) . " sudah digunakan!", "error");
     	return header("Location: uang_kas.php");
	} else {
		$query = mysqli_query($conn, "INSERT INTO bulan_pembayaran VALUES ('', '$nama_bulan', '$tahun', '$pembayaran_perbulan')");
		$id_bulan_pembayaran = mysqli_insert_id($conn);

		$dataSiswa = mysqli_query($conn, "SELECT * FROM siswa");
		$siswa = "INSERT INTO uang_kas (id_siswa, id_bulan_pembayaran, kas) VALUES ";
		foreach ($dataSiswa as $ds) {
		 	$siswa .= "('{$ds['id_siswa']}','{$id_bulan_pembayaran}','0')";
            $siswa .= ",";
		}
		$siswa = rtrim($siswa,",");
...
function editBulanPembayaran($data) {
	...
	$tahun = htmlspecialchars($data['tahun']);
	$pembayaran_perbulan = htmlspecialchars($data['pembayaran_perbulan']);
	$query = mysqli_query($conn, "UPDATE bulan_pembayaran SET nama_bulan = '$nama_bulan', tahun = '$tahun', pembayaran_perbulan = '$pembayaran_perbulan' WHERE id_bulan_pembayaran = '$id_bulan_pembayaran'");
  	return mysqli_affected_rows($conn);
}
...
function editPembayaranUangKas($data) {
	...
	if (isset($_POST['kas'])) {
		...
		$kas = htmlspecialchars($_POST['kas']);
		$query = mysqli_query($conn, "UPDATE uang_kas SET kas = '$kas' WHERE id_uang_kas = '$id_uang_kas'");
  		riwayat($id_user, $id_uang_kas, "telah mengubah pembayaran kas dari Rp. " . number_format($uang_sebelum) . " menjadi Rp. " . number_format($kas));
  		return mysqli_affected_rows($conn);
	}
}
...
function changePassword($data) {
	...
	if (password_verify($old_password, $dataUser['password'])) {
		...
		if ($new_password == $new_password_verify) {
			...
			setAlert("Gagal mengubah password pengguna!", "Password baru tidak cocok dengan password verify!", "error");
	     	return header("Location: profile.php");
		}
	} else {
		setAlert("Gagal mengubah password pengguna!", "Password lama tidak cocok!", "error");
     	return header("Location: profile.php");
	}
}
...
function tambahSiswaUangKas($data) {
	...
	$id_siswa = htmlspecialchars($data['id_siswa']);
	$id_bulan_pembayaran = htmlspecialchars($data['id_bulan_pembayaran']);
	$query = mysqli_query($conn, "INSERT INTO uang_kas VALUES ('', '$id_siswa', '$id_bulan_pembayaran', '0', '0')");
  	return mysqli_affected_rows($conn);
}
